...
#### `diffObject` - Fixed comparison logic

v1.x compared *values* instead of *keys*, producing incorrect results.

```typescript
// v1.x (broken) - compared values, not keys
diffObject({ a: 1, b: 2 }, { a: 1, b: 3 });
// Incorrect behavior

// v2.0 (fixed) - properly compares by key using Object.is
diffObject({ a: 1, b: 2 }, { a: 1, b: 3 });
// { b: 2 }
```

#### `getDifferenceArray` - Preserves types

v1.x used object-key indexing, converting all values to strings.

```typescript
// v1.x - returned string[]
getDifferenceArray([1, 2, 3], [2, 3, 4]); // ['1', '4']

// v2.0 - returns T[] (preserves original types)
getDifferenceArray([1, 2, 3], [2, 3, 4]); // [1, 4]
```

#### `preventMultipleExecution` - Returns wrapper function

v1.x used global state (shared across all usages). v2.0 returns a wrapper function with isolated state.

```typescript
// v1.x (broken)
preventMultipleExecution(myFn, 1000); // Direct call with global state

// v2.0 (fixed)
const safeFn = preventMultipleExecution(myFn, 1000);
safeFn(); // Each wrapper has independent state
```

#### `removeTag` - Renamed to `stripSimpleHtmlTags`

```typescript
// v1.x
import { removeTag } from 'ts-essentials-functions';

// v2.0
import { stripSimpleHtmlTags } from 'ts-essentials-functions';
// `removeTag` still works as deprecated alias
```

#### `log` - Removed

The `log` utility (colorized console.log) has been removed. Use dedicated logging libraries instead:

```typescript
// v1.x
import { log } from 'ts-essentials-functions';

// v2.0 - use a logging library
